...
class SiteController extends Controller
{
        ...
        if (isset($_POST['ContactForm'])) {
            ...
            if ($model->validate()) {
                ...
                if ((new Mailer())->mail($emails, 'پیام از بخش تماس با ما وبسایت ' . Yii::app()->name, $msg,
                    ...
                    Yii::app()->user->setFlash('footer-failed',
                        'متاسفانه ارسال پیام با مشکل مواجه است. لطفا مجددا تلاش کنید.');
                    if (isset($_POST['ajax'])) {
    ...
    public function actionGuidance()
    {
        ...
        $this->render('//site/pages/guidance',
            array('dataProvider' => $dataProvider, 'dataProvider2' => $dataProvider2));
    }
        ...
        if (isset($_GET['SearchForm'])) {
            ...
            switch ($model->type) {
                ...
                default:
                    ...
                    $dataProviders = [];
                    // courses search
                    $criteria                                            = Courses::getSearchCriteria($model->text,
                        $words);
                    $dataProviders['courses']['dataProvider']['courses'] = new CArrayDataProvider(Courses::model()->findAll($criteria),
                        array(
                            'pagination' => array('pageSize' => $pageSize)
                        ));
                    // files search
                    $criteria                                          = ClassCategoryFiles::getSearchCriteria($model->text,
                        $words);
                    $dataProviders['courses']['dataProvider']['files'] = new CActiveDataProvider('ClassCategoryFiles',
                        array(
                            'criteria'   => $criteria,
                            'pagination' => array('pageSize' => $pageSize)
                        ));
                    // links search
                    $criteria                                          = ClassCategoryFileLinks::getSearchCriteria($model->text,
                        $words);
                    $dataProviders['courses']['dataProvider']['links'] = new CActiveDataProvider('ClassCategoryFileLinks',
                        array(
                            'criteria'   => $criteria,
                            'pagination' => array('pageSize' => $pageSize)
                        ));
                    ...
                    // article search
                    $criteria                                              = Articles::getSearchCriteria($model->text,
                        $words);
                    ...
                    // files search
                    $criteria                                           = ArticleFiles::getSearchCriteria($model->text,
                        $words);
                    ...
                    // links search
                    $criteria                                           = ArticleFileLinks::getSearchCriteria($model->text,
                        $words);
                    $dataProviders['articles']['dataProvider']['links'] = new CActiveDataProvider('ArticleFileLinks',
                        array(
                            'criteria'   => $criteria,
                            'pagination' => array('pageSize' => $pageSize)
                        ));
                    ...
                    // ext links search
                    $criteria                                              = ArticleLinks::getSearchCriteria($model->text,
                        $words);
                    $dataProviders['articles']['dataProvider']['extLinks'] = new CActiveDataProvider('ArticleLinks',
                        array(
                            'criteria'   => $criteria,
                            'pagination' => array('pageSize' => $pageSize)
                        ));
    ...
    public function actionDownloadFromGoogle()
    {
        ...
        try {
            ...
            if ($copy) {
                ...
                } elseif ($move->model == 'ArticleFileLinks') {
