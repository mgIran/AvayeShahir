        ...
        public function afterFind($event)
        {
                ...
                if ( in_array( strtolower(Yii::app()->controller->route), array_map('strtolower', $this->admin_routes) ) ) 
                {
                        ...
                        foreach ($this->translated_attributes as $attr) 
                        {
                                foreach ($languages as $lang) 
                                {
                                        if (isset($result[$this->owner->primaryKey][$attr][$lang]))
                                        {
                                                $this->owner->{$attr.'_'.$lang} = $result[$this->owner->primaryKey][$attr][$lang];
                                        }
                                }
                        }
                }
                ...
                elseif (in_array(Yii::app()->language, $languages))
                {
                        ...
                        foreach ($this->translated_attributes as $attr) 
                        {
                                if ( isset($result[$this->owner->primaryKey][$attr]) AND ! empty($result[$this->owner->primaryKey][$attr]) )
                                {
                                        $this->owner->{$attr} = $result[$this->owner->primaryKey][$attr];
                                }
                        }
                }
