...
class Multimedia extends SortableCActiveRecord
{
    ...
    public function behaviors()
    {
        return array(
            'EasyMultiLanguage' => array(
                ...
                'admin_routes'          => array(
                    'multimedia/videos/create',
                    'multimedia/videos/update',
                    'multimedia/videos/admin',
                    'multimedia/pictures/create',
                    'multimedia/pictures/update',
                    'multimedia/pictures/admin'
                ),
                //
    ...
    public static function getLatest($type = 'videos', $limit = 15, $return = 'html')
    {
        $criteria = new CDbCriteria();
        if ($type == 'videos') {
            $criteria->addCondition('type', '!=', Multimedia::TYPE_PICTURE);
        } else {
            $criteria->compare('type', Multimedia::TYPE_PICTURE);
        }
        $criteria->limit = $limit;
        $criteria->order = 't.order ASC';
        if ($return == 'html') {
            foreach (Multimedia::model()->findAll($criteria) as $model) {
                echo '<li><a href="' . Yii::app()->createUrl('/multimedia/' . $type . '/' . $model->id . '/' . urlencode(str_replace('.',
                            '', $model->title))) . '" ><span>' . $model->title . '</span></a></li>';
            }
        } else {
            $out = array();
            foreach (Multimedia::model()->findAll($criteria) as $model) {
