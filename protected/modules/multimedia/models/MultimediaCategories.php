...
/**
 * This is the model class for table "{{multimedia_categories}}".
 *
 * The followings are the available columns in table '{{multimedia_categories}}':
 * @property string $id
 ...
 * @property MultimediaCategories $parent
 * @property MultimediaCategories[] $childes
 */
class MultimediaCategories extends SortableCActiveRecord
{
	/**
	...
	public function behaviors()
	{
		return array(
			'EasyMultiLanguage' => array(
				...
				'admin_routes' => array('multimedia/category/admin', 'multimedia/category/update', 'multimedia/category/delete', 'multimedia/category/create'),
				//
	...
	public function compareWithParent($attribute, $params)
	{
		if(!empty($this->title) && $this->parent_id){
			$record = MultimediaCategories::model()->findByAttributes(array('id' => $this->parent_id, 'title' => $this->title));
			if($record)
	...
	public function relations()
	{
		...
		return array(
			'multimedia' => array(self::HAS_MANY, 'Multimedia', 'category_id'),
			'parent' => array(self::BELONGS_TO, 'MultimediaCategories', 'parent_id'),
			'childes' => array(self::HAS_MANY, 'MultimediaCategories', 'parent_id'),
		);
	}
	 ...
	 * @return MultimediaCategories the static model class
	 */
	...
	private function updatePath($id)
	{
		...
		if($model->parent){
			...
			MultimediaCategories::model()->updateByPk($model->id, array('path' => $path));
		}
	...
	{
		foreach(MultimediaCategories::model()->getParents($categoryID, 'title') as $id => $title){
			echo '<li class="' . ($activeID == $id?'active':'') . '" ><a href="' . Yii::app()->createUrl('/multimedia/category/' . $id . '/' . urlencode($title)) . '" ><span>' . $title . '</span>&nbsp;&nbsp;<small>(' . self::model()->countMultimedia($id) . ')</small></a></li>';
			if(MultimediaCategories::model()->count('parent_id = :id', array(':id' => $id))){
				echo '<ol>';
	...
	public function countMultimedia($id = NULL)
	{
		$criteria = new CDbCriteria();
		$criteria->addInCondition('category_id', MultimediaCategories::model()->getCategoryChildes($id));
		return Multimedia::model()->count($criteria);
	}
}