...
/**
 * This is the model class for table "{{writing_categories}}".
 *
 * The followings are the available columns in table '{{writing_categories}}':
 * @property string $id
...
class WritingCategories extends SortableCActiveRecord
{
	...
	public function behaviors()
	{
		return array(
			'EasyMultiLanguage' => array(
				...
				'admin_routes' => array('writings/category/admin' ,'writings/category/update' ,'writings/category/delete' ,'writings/category/create') ,
				//
	...
	public function compareWithParent($attribute ,$params)
	{
		if(!empty($this->title) && $this->parent_id){
			$record = WritingCategories::model()->findByAttributes(array('id' => $this->parent_id ,'title' => $this->title));
			if($record)
	...
	public function relations()
	{
		...
		return array(
			'writings' => array(self::HAS_MANY, 'Writings', 'category_id'),
			'childes' => array(self::HAS_MANY, 'WritingCategories', 'parent_id'),
			'parent' => array(self::BELONGS_TO, 'WritingCategories', 'parent_id'),
			'tags' => array(self::MANY_MANY, 'ClassTags', '{{writing_category_tag_rel}}(category_id,tag_id)'),
			'tagsRel' => array(self::HAS_MANY, 'WritingCategoryTagRel', 'writing_id'),
		);
	}
	 ...
	 * @return WritingCategories the static model class
	 */
	...
	protected function afterSave()
	{
		if($this->formTags && !empty($this->formTags)){
			if(!$this->isNewRecord)
				WritingCategoryTagRel::model()->deleteAll('category_id=' . $this->id);
			foreach($this->formTags as $tag){
				$tagModel = ClassTags::model()->findByAttributes(array('title' => $tag));
				if($tagModel){
					$tag_rel = new WritingCategoryTagRel();
	...
	private function updatePath($id)
	{
		...
		$model = WritingCategories::model()->findByPk($id);
		if($model->parent){
			$path = $model->parent->path ? $model->parent->path . $model->parent_id . '-' : $model->parent_id . '-';
			WritingCategories::model()->updateByPk($model->id ,array('path' => $path));
		}
	...
	{
		foreach(WritingCategories::model()->getParents($categoryID ,'title', 'object') as $category){
			echo '<li class="' . ($activeID == $category->id ? 'active' : '') . '" ><a href="' . Yii::app()->createUrl('/writings/category/' . $category->id . '/' . urlencode($category->getValueLang('title','en'))) . '" ><span>' . $category->title . '</span>&nbsp;&nbsp;<small>(' . WritingCategories::model()->countWritings($category->id) . ')</small></a></li>';
			if(WritingCategories::model()->count('parent_id = :id' ,array(':id' => $category->id))){
				echo '<ol>';
	...
	public function countWritings($id = NULL)
	{
		$criteria = Writings::getValidWritings();
		$criteria->addInCondition('category_id' ,WritingCategories::model()->getCategoryChildes($id));
		return Writings::model()->count($criteria);
	}
