...
class WritingsCategoryController extends Controller
{
	/**
	...
	public function actionView($id)
	{
		...
		$this->keywords = 'آوای شهیر,رایتینیگ ها آموزشی,دسته بندی رایتینیگ ها آموزشی,دسته بندی '.$model->title.','.$model->title;
        ...
        $categoryProvider = new CActiveDataProvider('WritingCategories',array(
	...
	public function actionCreate()
	{
        ...
        $imageDIR = Yii::getPathOfAlias("webroot") . "/uploads/writings/categories/";
        if (!is_dir($imageDIR))
            mkdir($imageDIR);
        if (!is_dir($imageDIR.'/80x80'))
            mkdir($imageDIR.'/80x80');

		$model=new WritingCategories;
        
        $image = array();
        
		if(isset($_POST['WritingCategories']))
		{
			$model->attributes=$_POST['WritingCategories'];
            if (isset($_POST['WritingCategories']['image'])) {
                $file = $_POST['WritingCategories']['image'];
                $image = array(
                    'name' => $file,
                    'src' => $tmpUrl . '/' . $file,
            ...
            $model->formTags = isset($_POST['WritingCategories']['formTags'])?explode(',',$_POST['WritingCategories']['formTags']):null;
			if($model->save()) {
	...
	public function actionUpdate($id)
	{
        ...
        $tmpUrl = Yii::app()->baseUrl .'/uploads/temp/';
        $imageDIR = Yii::getPathOfAlias("webroot") . "/uploads/writings/categories/";
        $imageUrl = Yii::app()->baseUrl .'/uploads/writings/categories/';
		...
		{
			$model->attributes=$_POST['WritingCategories'];
            if (isset($_POST['WritingCategories']['image']) and file_exists($tmpDIR.$_POST['WritingCategories']['image'])) {
                $file = $_POST['WritingCategories']['image'];
                $image = array(
                    'name' => $file,
                    'src' => $tmpUrl . '/' . $file,
	...
	{
		$dataProvider=new CActiveDataProvider('WritingCategories');
	...
	{
		$model=new WritingCategories('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['WritingCategories']))
			$model->attributes=$_GET['WritingCategories'];
	 ...
	 * @return WritingCategories the loaded model
	 ...
	 * @param WritingCategories $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='writings-categories-form')
		{
	...
	{
		$Dir = Yii::getPathOfAlias("webroot") . '/uploads/writings/categories/';
		...
		if (isset($_POST['fileName'])) {
			...
			$model = WritingCategories::model()->findByAttributes(array('image' => $fileName));
			if ($model) {
