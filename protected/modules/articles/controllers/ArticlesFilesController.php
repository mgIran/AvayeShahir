...
class ArticlesFilesController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['ArticleFiles'])) {
			...
			if($model->save()) {
				...
				$response = ['state' => 'ok','url'=>Yii::app()->createUrl('/articles/manage/update/id/'.$model->article_id.'/step/2')];
			} else
			{
				Yii::app()->user->setFlash('upload-failed', 'در ثبت اطلاعات خطایی رخ داده است! لطفا مجددا تلاش کنید.');
				$response = ['state' => 'error','url'=>Yii::app()->createUrl('/articles/manage/update/id/'.$model->article_id.'/step/2')];
			}
	...
	{
		$model = ArticleFiles::model()->findByPk((int)$id);
		...
		if(isset($_POST['ArticleFiles'])) {
			...
			if($model->save()) {
				...
				Yii::app()->user->setFlash('upload-failed', 'در ثبت اطلاعات خطایی رخ داده است! لطفا مجددا تلاش کنید.');
			$this->redirect(array('/articles/manage/update/id/'.$model->article_id.'/step/2'));
		}
	...
	public function actionDeleteUpload()
	{
		...
		if (isset($_POST['fileName'])) {
			...
			$model = ArticleFiles::model()->findByAttributes(array('path' => $fileName));
			if ($model) {
	...
	public function actionUploadImage()
	{
		...
		if (isset($_FILES)) {
			...
			if ($model && move_uploaded_file($file['tmp_name'], $tempDir . DIRECTORY_SEPARATOR . strip_tags($file['name']))){
				ArticleFiles::model()->updateByPk($model->id,array('image'=> strip_tags($file['name'])));
				$response = ['state' => 'ok', 'fileName' => CHtml::encode($file['name'])];
			}
			else
				$response = ['state' => 'error', 'msg' => 'فایل آپلود نشد.'];
	...
	public function actionDeleteUploadImage()
	{
		...
		if (isset($_POST['fileName'])) {
			...
			$fileName = $_POST['fileName'];
			$model = ArticleFiles::model()->findByAttributes(array('image' => $fileName));
			if ($model) {
