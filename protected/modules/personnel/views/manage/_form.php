		...
		<div class="dynamic-box">
			...
			if($model->social_links):
				...
				<div class="dynamic-field input-group">
					...
					<div class="input">
						<?= CHtml::textField('Personnel[social_links][0][value]',(isset($social_links[0])?$social_links[0]['value']:''),array('class'=>'form-control','placeholder'=>'آدرس لینک')); ?>
					</div>
				</div>
				<div class="dynamic-field input-group">
					<div class="label">توییتر : </div>
					<div class="input">
						<?= CHtml::textField('Personnel[social_links][1][value]',(isset($social_links[1])?$social_links[1]['value']:''),array('class'=>'form-control','placeholder'=>'آدرس لینک')); ?>
					</div>
				</div>
				<?
				...
				foreach($social_links as $key => $link):
					...
					<div class="dynamic-field input-group">
						<div class="label">
							<?= CHtml::textField("Personnel[social_links][$key][title]",$link['title'],array('class'=>'form-control')); ?>
						</div>
						<div class="input">
							<?= CHtml::textField("Personnel[social_links][$key][value]",$link['value'],array('class'=>'form-control')); ?>
						</div>
					</div>
					<?
				endforeach;
				...
				<div class="dynamic-field input-group">
					<div class="label">فیسبوک : </div>
					<div class="input">
						<?= CHtml::textField('Personnel[social_links][0][value]','',array('class'=>'form-control' ,'placeholder'=>'آدرس لینک')); ?>
					</div>
				</div>
				<div class="dynamic-field input-group">
					<div class="label">توییتر : </div>
					<div class="input">
						<?= CHtml::textField('Personnel[social_links][1][value]','',array('class'=>'form-control','placeholder'=>'آدرس لینک')); ?>
					</div>
				</div>
				<?
			endif;
...
Yii::app()->clientScript->registerScript('dynamic-fields','
		...
		$(\'.add-dynamic-field-trigger\').click(function () {
			...
			box.append("<div class=\'dynamic-field input-group\'>" +
				"<div class=\'label\'>" +
					"<input type=\'text\' name=\'"+"Personnel[social_links]["+$dynamicFieldsNum+"][title]"+"\' class=\'form-control\' placeholder=\'عنوان شبکه اجتماعی\' >" +
				"</div>" +
				"<div class=\'input\'>" +
					"<input type=\'text\' name=\'"+"Personnel[social_links]["+$dynamicFieldsNum+"][value]"+"\' class=\'form-control\' placeholder=\'آدرس لینک\' >" +
				"</div> " +
			"</div>");
		});
