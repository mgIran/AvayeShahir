...
<head>
    ...
    $cs->registerCssFile($baseUrl.'/css/svg.css?8.8');
    $cs->registerCssFile($baseUrl.'/css/bootstrap-theme.css?8.8');
    $cs->registerCssFile($baseUrl.'/css/responsive-theme.css?8.8');
    if (Yii::app()->params['default_language'] !== Yii::app()->language)
    {
        // @todo add css for multi language
        $cs->registerCssFile($baseUrl.'/css/bootstrap-theme-'.Yii::app()->language.'.css?8.8');
    }
