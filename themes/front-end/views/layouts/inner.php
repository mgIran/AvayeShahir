...
<head>
    ...
    $cs->registerCssFile($baseUrl.'/css/svg.css?7.5');
    $cs->registerCssFile($baseUrl.'/css/bootstrap-theme.css?7.5');
    $cs->registerCssFile($baseUrl.'/css/responsive-theme.css?7.5');
    if (Yii::app()->params['default_language'] !== Yii::app()->language)
    {
        // @todo add css for multi language
        $cs->registerCssFile($baseUrl.'/css/bootstrap-theme-'.Yii::app()->language.'.css?7.5');
    }
    ...
    $cs->registerScriptFile($baseUrl.'/js/scripts.js?7.5');
    ?>
</head>
