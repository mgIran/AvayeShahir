...
<head>
    ...
    $cs->registerCssFile($baseUrl.'/css/svg.css?5.7');
    $cs->registerCssFile($baseUrl.'/css/bootstrap-theme.css?5.7');
    $cs->registerCssFile($baseUrl.'/css/responsive-theme.css?5.7');
    if (Yii::app()->params['default_language'] !== Yii::app()->language)
    {
        // @todo add css for multi language
        $cs->registerCssFile($baseUrl.'/css/bootstrap-theme-'.Yii::app()->language.'.css?5.7');
    }
    ...
    $cs->registerScriptFile($baseUrl.'/js/scripts.js?5.7');
    ?>
</head>
