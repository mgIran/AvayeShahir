...
<head>
    ...
    $cs->registerCssFile($baseUrl.'/css/svg.css?6.7');
    $cs->registerCssFile($baseUrl.'/css/bootstrap-theme.css?6.7');
    $cs->registerCssFile($baseUrl.'/css/responsive-theme.css?6.7');
    if (Yii::app()->params['default_language'] !== Yii::app()->language)
    {
        // @todo add css for multi language
        $cs->registerCssFile($baseUrl.'/css/bootstrap-theme-'.Yii::app()->language.'.css?6.7');
    }
    ...
    $cs->registerScriptFile($baseUrl.'/js/scripts.js?6.7');
    ?>
</head>
