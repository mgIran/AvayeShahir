...
<head>
    ...
    $cs->registerCssFile($baseUrl.'/css/bootstrap-theme.css?9.4');
    $cs->registerCssFile($baseUrl.'/css/responsive-theme.css?9.4');
    if (Yii::app()->params['default_language'] !== Yii::app()->language)
    {
        // @todo add css for multi language
        $cs->registerCssFile($baseUrl.'/css/bootstrap-theme-'.Yii::app()->language.'.css?9.4');
    }
    $cs->registerCssFile($baseUrl.'/css/panel.css?9.4');
    if (Yii::app()->params['default_language'] !== Yii::app()->language)
    {
        // @todo add css for multi language
        $cs->registerCssFile($baseUrl.'/css/panel-'.Yii::app()->language.'.css?9.4');
    }
    $cs->registerCssFile($baseUrl.'/css/panel-responsive-theme.css?9.4');
    ...
    $cs->registerScriptFile($baseUrl.'/js/scripts.js?9.4');
    ?>
</head>
